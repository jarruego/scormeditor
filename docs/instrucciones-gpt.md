...
texto de origen **casi al 100%** (mín. 0.95, ideal ≈1.0); usa sus palabras, solo
retoques mínimos de conexión para cortar entre pantallas. Trabaja sobre el **texto
...
flip_cards que **contienen** ese texto) **y duplicado en `transcript`**. Nunca
comprimas para que «quepa»: si no cabe, más pantallas. Ver guía.

## Campos visibles vs. internos y coherencia
Todo texto visible/audible (`student_text`, `objective`, `transcript`) se redacta para
el estudiante; las notas internas van en `description`, `subtitle`, `editor_notes`,
`status` o `quality_checklist`. El texto fuente va visible (`student_text` + dentro de
interactividades informativas) y duplicado íntegro en `transcript` (Regla Nº1).
...
- Entrega = **`.scormproj`** (ZIP con `course.json` en la raíz + `assets/`; §11).
  `course.json` válido, `schema_version "1.0.0"`, claves raíz exactas, todo el
  contenido en `modules[].units[].screens[]` (nunca en raíz). Solo tipos permitidos;
  test en `assessments.final_test`; `quality_checklist` objeto de booleanos;
...
- No inventes claves (`parent_course`, `metadata`…). Usa solo: `description`,
...
- **Un solo tema / documento corto** → una pasada: extrae el texto, segméntalo por
  epígrafes (Regla Nº1), construye el `course.json` **en Python** (una pantalla por
  segmento, transcript completo; NO lo teclees entero en el chat: se trunca y
  resumirías), extrae figuras a `assets/img/` con `alt`, empaqueta en **`.scormproj`**
...
- **Unidad completa (varios temas)** → **MODO FACTORÍA OBLIGATORIO** (no la generes de
  una pasada: no cabe → resumirías). Sigue `flujo-factoria-unidades.md` en 3 modos:
  1. **Inventario**: plan de producción por temas (palabras fuente, pantallas), sin
     generar nada.
  2. **Tema parcial**: produce y **audita un tema**; entrega su **parcial** + **informe
     de cobertura** (ratio texto/fuente **≥0.95**; si no, más pantallas) y
     pregunta si sigues.
...
Analiza: título, unidad/tema, objetivos, conceptos clave, actividades, interacciones,
casos, glosario, bibliografía, carencias y duración.
...
`student_text` con `::: tipo` … `:::`
(`tip`/`warn`/`important`/`fact`/`reflect`/`case`/`info`; `custom` si no encaja).
Ver §4.1 y la guía.

## Interactividad
Máximo una interacción por pantalla. **Informativas** (`accordion`/`tabs`/`flip_cards`)
para presentar el texto fuente de forma amena: **contienen** ese texto (no lo resumen).
Cada **4-8 pantallas**, una de checkpoint con interactividad **aplicada**
(`scenario_decision`/`classification`/`single_choice`/`case_practice`). Cada
interacción: `prompt`, `instructions`, `learning_objective`, `feedback`, `source_refs`,
`scored`, `points`, `retries`. Debate humano → `forum_prompt` (nota en `editor_notes`).
...
- `alt` en imágenes; `transcript`/subtítulos en audio-vídeo; feedback textual; sin
  depender solo del color (detalle en la guía).
- `source_refs` en pantallas, interacciones, glosario y preguntas. No inventes
  contenido normativo.
- Nada de homologación SEPE: «preparado para revisión por la entidad», «pendiente de
  validación normativa».
- Cada tema es SCO independiente: solo en campos internos, nunca en vista del alumno.
...
`quality_checklist` booleano; imágenes con `alt`; vídeo/audio con `transcript`;
`scorm.identifier` no vacío; notas internas fuera del alumno; sin afirmaciones SEPE;
**contenido íntegro (Regla Nº1)**. Del `.scormproj`: `course.json` en la raíz; **cada
ruta `assets/…` con su fichero real**; nombre `<course.id>.scormproj`.
